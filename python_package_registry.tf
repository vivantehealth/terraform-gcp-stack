...
resource "google_artifact_registry_repository" "stack_repo" {
  count = length(var.python_registry) > 0 && (var.env_id == "dev" || var.env_id == "tools-dev") ? 1 : 0
  ...
  project       = one(regex("^[^/]+/([^/]+).*$", var.python_registry)) #can't be a "local" as written
  location      = "us"
...
resource "google_artifact_registry_repository_iam_member" "iac_admin" {
  count = length(var.python_registry) > 0 ? 1 : 0

  project    = one(regex("^[^/]+/([^/]+).*$", var.python_registry))
  location   = "us"
  repository = "projects/${one(regex("^[^/]+/([^/]+).*$", var.python_registry))}/locations/us/repositories/${var.repo}-python-packages"
